...
class HessianPointValidator(HessianValidator):
    ...
    def _compute_finite_difference_results(self, points: torch.Tensor, energy_func: Callable) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        for idx in range(N):
            for space_x in range(n):
                ...
                f_plus  = energy_func((points + perturb)[0], (points + perturb)[1])
                f_minus = energy_func((points - perturb)[0], (points - perturb)[1])
        ...
        for jdx in range(N):
            for space_jx in range(n):
                ...
                f_plus  = energy_func.analytical_force((points + perturb)[0], (points + perturb)[1])
                f_minus = energy_func.analytical_force((points - perturb)[0], (points - perturb)[1])
    ...
    def _compute_automatic_diff_results(self, points: torch.Tensor, energy_func: Callable) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        # 计算梯度
        energy = energy_func(points[0], points[1])
