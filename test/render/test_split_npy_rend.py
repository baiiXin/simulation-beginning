...
def load_triangles(nm, script_dir):
    candidates = [
...
def render_image_polyscope(data, idx, out_dir, base_name, width, height, tris):
...
def render_video_polyscope(data, indices, out_dir, base_name, fps, width, height, tris):
    os.makedirs(out_dir, exist_ok=True)
    try:
        import polyscope as ps
    except Exception:
...
def main():
    ...
    default_file = os.path.join(default_dir, "cloth_data_k80.npy")
    ...
    tris = load_triangles(M, default_dir)

    if args.mode == "image":
        indices = indices[:1]
        render_image_polyscope(data, indices[0], out_dir, base_name, args.width, args.height, tris)
    else:
        render_video_polyscope(data, indices, out_dir, base_name, args.fps, args.width, args.height, tris)
